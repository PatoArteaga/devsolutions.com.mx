...
if ($idSelectPais > 0) {
    $capital = "SELECT id_capital, nombre_capital FROM capitales WHERE id_pais = $idSelectPais";
    ...
    if (mysqli_num_rows($ciudad) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($ciudad)) {
            echo '<option value="' . htmlspecialchars($row['id_capital']) . '">' . htmlspecialchars($row['nombre_capital']) . '</option>';
        }
    } else {
